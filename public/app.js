...
async function startSlowJob(step, slowCfg) {
  ...
  try {
    const r = await fetch('/api/chat/async', { method: 'POST', headers: await authHeaders(), body: JSON.stringify({ messages: msgsForServer, currentStep: step, stepContent: stepContentStore(), sectionContent: sectionStore() }) });
...
chatForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const r = await fetch('/api/chat', {
      method: 'POST',
      headers,
      body: JSON.stringify({ messages: history, currentStep: step, stepContent: stepContentStore(), sectionContent: sectionStore() })
    });
    const data = await r.json();
